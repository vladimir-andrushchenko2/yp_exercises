...
public:
    ...
    string ToString() const {
        ...
        // чтобы дополнить цифровую часть номера слева нулям
...
public:
    ...
    int64_t GetCurrentBill(const VehiclePlate& car) const {
        ...
        */
        ...
        if (complete_parks_.count(car) > 0) {
            duration += complete_parks_.at(car);
        }
    ...
    unordered_map<VehiclePlate, int64_t, VehiclePlateHasher> EndPeriodAndGetBills() {
        for (auto& [car, time] : now_parked_) {
            complete_parks_[car] += Clock::now() - time;
            time = Clock::now();
        }

        unordered_map<VehiclePlate, int64_t, VehiclePlateHasher> output;
